...
urlpatterns = [
    path('', include('apps.mainpage.urls'), name='inicio'),
    path('admin/', admin.site.urls),
    path('login/', include('apps.login.urls'), name='login'),
    path('reset/password_reset', PasswordResetView.as_view(template_name='password/password_reset_form.html',
    ...
    re_path(r'^reset/(?P<uidb64>[0-9A-za-z_\-]+)/(?P<token>.+)/$',
            PasswordResetConfirmView.as_view(template_name='password/password_reset_confirms.html'),
